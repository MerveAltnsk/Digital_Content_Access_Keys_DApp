...
export class DigitalAccessKeysContract {
  ...
  async getBalance(address: string): Promise<number> {
    try {
      ...
      if (result.result && result.result.retval) {
        return parseInt(result.result.retval.toString());
      }
      return 0;
    } catch (error) {
  ...
  async freezeAccount(account: string, freeze: boolean): Promise<string> {
    const operation = this.contract.call(
      "freeze_account",
      new Address(account).toScVal(),
      xdr.ScVal.scvBool(freeze)
    );
  ...
  async isFrozen(account: string): Promise<boolean> {
    try {
      ...
      if (result.result && result.result.retval) {
        return result.result.retval.toString() === "true";
      }
      return false;
    } catch (error) {
  ...
  async setContentMetadata(
    ...
    const operation = this.contract.call(
      ...
      xdr.ScVal.scvI128(xdr.Int128.fromString(price.toString())),
